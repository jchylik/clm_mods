...
#include "shr_assert.h"
  ...
  real(r8), parameter  :: const_beccs_fac       = 0.65_r8  ! #beccs for testing only,--> replace with time and index dependent
  real(r8), parameter  :: mfrac_c               = 0.49_r8  ! #beccs carbon in beccs dry matter, replace with pft dependent 
  real(r8), parameter  :: c2co2_c               = 3.67_r8  ! #beccs carbon dioxide to carbon ratio
...
contains
  ...
  subroutine CNPhenology (bounds, num_soilc, filter_soilc, num_soilp, &
    ...
    if ( phase == 1 ) then
       ...
       call CNCropHarvestToProductPools(bounds, num_soilp, filter_soilp, num_soilc, filter_soilc, &
            cnveg_carbonflux_inst, cnveg_nitrogenflux_inst)  !default: call CNGrainToProductPools(bounds, num_soilp, filter_soilp, num_soilc, filter_soilc, &
  ...
  subroutine CNOffsetLitterfall (num_soilp, filter_soilp, &
    ...
    real(r8) :: beccs_frac         ! fraction of harvest for biofuels that goes into beccs
    !-----------------------------------------------------------------------
    ...
    associate(                                                                           & 
         ...
         leafc_to_beccsc       =>    cnveg_carbonflux_inst%leafc_to_beccsc_patch       , & ! Output: [real(r8) (:) ]  leaf C to beccs C (gC/m2/s)
         livestemc_to_beccsc   =>    cnveg_carbonflux_inst%livestemc_to_beccsc_patch   , & ! Output: [real(r8) (:) ]  livestem C to beccs C (gC/m2/s)
         ...
         leafn_to_beccsn       =>    cnveg_nitrogenflux_inst%leafn_to_beccsn_patch     , & ! Output: [real(r8) (:) ]  leaf N to beccs N (gN/m2/s)
         livestemn_to_beccsn   =>    cnveg_nitrogenflux_inst%livestemn_to_beccsn_patch , & ! Output: [real(r8) (:) ]  livestem N to beccs N (gN/m2/s)              
      ...
      ! calculate 
      beccs_frac =  const_beccs_fac /(c2co2_c*mfrac_c)  ! #beccs -- later replace const_ with variable
         ...
         if (offset_flag(p) == 1._r8) then
            ...
            if (offset_counter(p) == dt) then
               ...
               if (ivt(p) >= npcropmin) then
                  ...
                  leafc_to_biofuelc(p) = t1 * leafc(p) * biofuel_harvfrac(ivt(p))*(1._r8-beccs_frac)
                  leafn_to_biofueln(p) = t1 * leafn(p) * biofuel_harvfrac(ivt(p))*(1._r8-beccs_frac)
                  leafc_to_beccsc(p)   = t1 * leafc(p) * biofuel_harvfrac(ivt(p))*beccs_frac
                  leafn_to_beccsn(p)   = t1 * leafn(p) * biofuel_harvfrac(ivt(p))*beccs_frac
                  ...
                  livestemc_to_biofuelc(p) = t1 * livestemc(p) * biofuel_harvfrac(ivt(p))*(1._r8-beccs_frac)
                  livestemn_to_biofueln(p) = t1 * livestemn(p) * biofuel_harvfrac(ivt(p))*(1._r8-beccs_frac)
                  livestemc_to_beccsc(p)   = t1 * livestemc(p) * biofuel_harvfrac(ivt(p))*beccs_frac
                  livestemn_to_beccsn(p)   = t1 * livestemn(p) * biofuel_harvfrac(ivt(p))*beccs_frac               
                  !---> add more beccs lines here
               end if
            else
            ...
            if (ivt(p) >= npcropmin) then
               ...
               livestemn_to_litter(p) = livestemn(p) / dt * (1._r8 - biofuel_harvfrac(ivt(p))) !livestemn_to_litter(p) = livestemn(p) / dt
            end if
  ...
  subroutine CNGrainToProductPools(bounds, num_soilp, filter_soilp, num_soilc, filter_soilc, &
       cnveg_carbonflux_inst, cnveg_nitrogenflux_inst) ! keeping for background compatibility
    !
    ! !DESCRIPTION:
  ...
  subroutine CNCropHarvestToProductPools(bounds, num_soilp, filter_soilp, num_soilc, filter_soilc, &
    ...
    if (use_crop) then
       do fp = 1, num_soilp     ! #beccs : not in 1-year pool
          p = filter_soilp(fp)
          ...
          ! #beccs instead goes to sequestred storage
          cnveg_carbonflux_inst%beccsc_to_prodseqc_patch(p)   = cnveg_carbonflux_inst%leafc_to_beccsc_patch(p) +  &
               cnveg_carbonflux_inst%livestemc_to_beccsc_patch(p)
          cnveg_nitrogenflux_inst%beccsn_to_prodseqn_patch(p) = cnveg_nitrogenflux_inst%leafn_to_beccsn_patch(p)+ &
               cnveg_nitrogenflux_inst%livestemn_to_beccsn_patch(p)
       end do
       ...
       call p2c (bounds, num_soilc, filter_soilc, &
            cnveg_carbonflux_inst%beccsc_to_prodseqc_patch(bounds%begp:bounds%endp), &
            cnveg_carbonflux_inst%beccsc_to_prodseqc_col(bounds%begc:bounds%endc))

       call p2c (bounds, num_soilc, filter_soilc, &
            cnveg_nitrogenflux_inst%beccsn_to_prodseqn_patch(bounds%begp:bounds%endp), &
            cnveg_nitrogenflux_inst%beccsn_to_prodseqn_col(bounds%begc:bounds%endc))            
    end if
