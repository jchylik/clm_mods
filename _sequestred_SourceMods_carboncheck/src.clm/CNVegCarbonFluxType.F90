...
#include "shr_assert.h"
  ...
  type, public :: cnveg_carbonflux_type
     ...
     real(r8), pointer :: leafc_to_beccsc_patch                     (:)     ! leaf C to beccs C (gC/m2/s)
     real(r8), pointer :: livestemc_to_beccsc_patch                 (:)     ! livestem C to beccs C (gC/m2/s)     
     ...
     real(r8), pointer :: beccsc_to_prodseqc_patch                  (:)     ! beccs C to sequestred product pool (gC/m2/s)
     real(r8), pointer :: beccsc_to_prodseqc_col                    (:)     ! beccs C to sequestred product pool (gC/m2/s)     
...
contains
  ...
  subroutine InitAllocate(this, bounds, carbon_type)
    ...
    allocate(this%leafc_to_beccsc_patch                     (begp:endp)) ; this%leafc_to_beccsc_patch                     (:) = nan
    allocate(this%livestemc_to_beccsc_patch                 (begp:endp)) ; this%livestemc_to_beccsc_patch                 (:) = nan   
    ...
    allocate(this%beccsc_to_prodseqc_patch(begp:endp)) ; this%beccsc_to_prodseqc_patch(:) = nan
    ...
    allocate(this%beccsc_to_prodseqc_col  (begc:endc)) ; this%beccsc_to_prodseqc_col  (:) = nan
  ...
  subroutine InitHistory(this, bounds, carbon_type)
    ...
    if (carbon_type == 'c12') then
       ...
       if (use_crop) then
          ...
          this%leafc_to_beccsc_patch(begp:endp) = spval                     ! #beccs
          call hist_addfld1d (fname='LEAFC_TO_BECCSC', units='gC/m^2/s', &
               avgflag='A', long_name='leaf C to beccs C', &
               ptr_patch=this%leafc_to_beccsc_patch)
               
          this%livestemc_to_beccsc_patch(begp:endp) = spval                  ! #beccs
          call hist_addfld1d (fname='LIVESTEMC_TO_BECCSC', units='gC/m^2/s', &
               avgflag='A', long_name='livestem C to beccs C', &
               ptr_patch=this%livestemc_to_beccsc_patch)           
...
! FUN Ends 
  ...
  subroutine SetValues ( this, &
    ...
    do fi = 1,num_patch
       ...
       this%beccsc_to_prodseqc_patch(i)                  = value_patch
    end do
    ...
    if ( use_crop )then
       do fi = 1,num_patch
          ...
          this%leafc_to_beccsc_patch(i)         = value_patch
          this%livestemc_to_beccsc_patch(i)     = value_patch
    ...
    do fi = 1,num_column
       ...
       this%beccsc_to_prodseqc_col(i)  = value_column
