  ...
  subroutine CIsoFlux1(num_soilc, filter_soilc, num_soilp, filter_soilp,         &
    ...
    associate(                                                            &
      ...
      if ( use_crop )then
         ...
         call CIsoFluxCalc(&
              iso_cnveg_cf%leafc_to_beccsc_patch               , cnveg_cf%leafc_to_beccsc_patch, &
              iso_cnveg_cs%leafc_patch                         , cnveg_cs%leafc_patch, &
              num_soilp                                        , filter_soilp, 1._r8, 0, isotope) 

         call CIsoFluxCalc(&
              iso_cnveg_cf%livestemc_to_beccsc_patch           , cnveg_cf%livestemc_to_beccsc_patch, &
              iso_cnveg_cs%livestemc_patch                     , cnveg_cs%livestemc_patch, &
              num_soilp                                        , filter_soilp, 1._r8, 0, isotope) 
         ...
         do fp = 1,num_soilp
            ...
            ! and similar update for beccs 
            iso_cnveg_cf%beccsc_to_prodseqc_patch(p) = iso_cnveg_cf%leafc_to_beccsc_patch(p) + &
                 iso_cnveg_cf%livestemc_to_beccsc_patch(p)            
         end do
