    ...
    if (.not. use_fates) then    
      do fp = 1,num_soilp
         ...
         if (ivt(p) >= npcropmin) then ! skip 2 generic crops
            ...
            cs_veg%livestemc_patch(p)  = cs_veg%livestemc_patch(p)  - (cf_veg%livestemc_to_biofuelc_patch(p)+cf_veg%livestemc_to_beccsc_patch(p))*dt
            cs_veg%leafc_patch(p)      = cs_veg%leafc_patch(p)      - (cf_veg%leafc_to_biofuelc_patch(p)+cf_veg%leafc_to_beccsc_patch(p))*dt
