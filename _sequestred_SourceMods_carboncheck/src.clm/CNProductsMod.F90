...
#include "shr_assert.h"
  ...
  type, public :: cn_products_type
     ...
     real(r8), pointer :: prodseq_grc(:)      ! (g[C or N]/m2) beccs product pool, sequestred storage
     ...
     real(r8), pointer :: hrv_beccs_to_prodseq_patch(:)  ! (g[C or N]/m2/s) beccs harvest to sequestred product pool
     real(r8), pointer :: hrv_beccs_to_prodseq_grc(:)    ! (g[C or N]/m2/s) beccs harvest to sequestred product pool
     ...
     real(r8), pointer :: prodseq_loss_grc(:)      ! (g[C or N]/m2/s) decomposition loss from sequestred product pool
   ...
   contains
     ...
     procedure, private :: PartitionCropProdFluxes
...
contains
  ...
  subroutine InitAllocate(this, bounds)
    ...
    allocate(this%prodseq_grc(begg:endg)) ; this%prodseq_grc(:) = nan
    ...
    allocate(this%hrv_beccs_to_prodseq_patch(begp:endp)) ; this%hrv_beccs_to_prodseq_patch(:) = nan
    allocate(this%hrv_beccs_to_prodseq_grc(begg:endg)) ;   this%hrv_beccs_to_prodseq_grc(:) = nan
    ...
    allocate(this%prodseq_loss_grc(begg:endg)) ; this%prodseq_loss_grc(:) = nan
  ...
  subroutine InitHistory(this, bounds)
    ...
    this%prodseq_grc(begg:endg) = spval
    call hist_addfld1d( &
         fname = this%species%hist_fname('PRODSEQ'), &
         units = 'g' // this%species%get_species() // '/m^2', &
         avgflag = 'A', &
         long_name = 'sequestred product pool' // this%species%get_species(), &
         ptr_gcell = this%prodseq_grc, default='inactive')
    ...
    this%prodseq_loss_grc(begg:endg) = spval
    call hist_addfld1d( &
         fname = this%species%hist_fname('PRODSEQ', suffix='_LOSS'), &
         units = 'g' // this%species%get_species() // '/m^2/s', &
         avgflag = 'A', &
         long_name = 'loss from sequestred product pool', &
         ptr_gcell = this%prodseq_loss_grc, default='inactive')
  ...
  subroutine InitCold(this, bounds)
    ...
    do p = bounds%begp, bounds%endp
       ...
       this%hrv_beccs_to_prodseq_patch(p)    = 0._r8
  ...
  subroutine Restart(this, bounds, ncid, flag, &
    ...
    call restartvar(ncid=ncid, flag=flag, &
         varname=this%species%rest_fname('prodseq', suffix='_g'), &
         xtype=ncd_double, dim1name='gridcell', &
         long_name='', units='', &
         interpinic_flag='interp', readvar=readvar, data=this%prodseq_grc)
    if (flag == 'read' .and. .not. readvar) then
       ! BACKWARDS_COMPATIBILITY(wjs, 2016-03-31) If the gridcell-level field isn't
       ! present, try to find a column-level field (which may be present on an older
       ! restart file).
       call set_grc_field_from_col_field( &
            bounds = bounds, &
            ncid = ncid, &
            varname = this%species%rest_fname('prodseq'), &
            data_grc = this%prodseq_grc, &
            readvar = readvar)

       ! If we still haven't found an appropriate field on the restart file, then set
       ! this field from the template, if provided
       if (.not. readvar .and. template_provided) then
          call set_missing_from_template(this%prodseq_grc, &
               template_for_missing_fields%prodseq_grc, &
               multiplier = template_multiplier)
       end if
    end if
  ...
  subroutine UpdateProducts(this, bounds, &
    ...
    ! other crop product pool (g/m2/s) [patch]  # beccs
    real(r8), intent(in) :: ocrop_to_cropprod_patch( bounds%begp: )
    !
    ! !LOCAL VARIABLES:
    integer  :: g        ! indices
    ...
    SHR_ASSERT_ALL((ubound(ocrop_to_cropprod_patch) == (/bounds%endp/)), errMsg(sourcefile, __LINE__))    
    ...
    ! #besccs -- later add switch above
    call this%PartitionCropProdFluxes(bounds, &
         num_soilp, filter_soilp, &
         dwt_crop_product_gain_patch(bounds%begp:bounds%endp), &
         grain_to_cropprod_patch(bounds%begp:bounds%endp),     &
         ocrop_to_cropprod_patch(bounds%begp:bounds%endp))    
    ...
    do g = bounds%begg, bounds%endg
       ...
       this%prodseq_loss_grc(g)   = 0._r8   ! --> later add loss from sequestred storage here
    end do
    ...
    do g = bounds%begg, bounds%endg
       ...
       this%prodseq_grc(g)   = this%prodseq_grc(g)   ! + this%dwt_beccs_to_prodseq_grc(g)*dt
       ...
       this%prodseq_grc(g)   = this%prodseq_grc(g)   + this%hrv_beccs_to_prodseq_grc(g)*dt
       ...
       this%prodseq_grc(g)   = this%prodseq_grc(g)   - this%prodseq_loss_grc(g)*dt
  ...
  !-----------------------------------------------------------------------
  subroutine PartitionCropProdFluxes(this, bounds, &
       num_soilp, filter_soilp, &
       dwt_crop_product_gain_patch, &
       grain_to_cropprod_patch, &
       cropother_to_cropprod_patch)
    !
    ! !DESCRIPTION:
    ! Partition input crop fluxes into crop product pools:
    !  * grain product fluxes
    !  * other product fluxes
    ! For now this doesn't do too much, since there is just one beccs product that 
    ! eventually goes into the sequestred storage pool, and a single (1-year) crop product
    ! pool. But this provides the capability to add different crop product pools in the
    ! future, without requiring any changes to code outside of this class. It also gives
    ! symmetry with the wood fluxes.
    !
    ...
    ! grain to crop product pool(s) (g/m2/s) [patch]
    real(r8), intent(in)    :: grain_to_cropprod_patch( bounds%begp: )
    ! other to crop product pool(s) (g/m2/s) [patch]
    real(r8), intent(in)    :: cropother_to_cropprod_patch( bounds%begp: )    
    !
    ! !LOCAL VARIABLES:
    integer :: fp
    integer :: p
    ...
    character(len=*), parameter :: subname = 'PartitionCropProdFluxes'
    !-----------------------------------------------------------------------
    ...
    do fp = 1, num_soilp
       ...
       ! For now all grain crop product is put in the 1-year crop product pool
       this%grain_to_cropprod1_patch(p) = grain_to_cropprod_patch(p)
       ! For now the other crop product is put in sequestred storage
       this%hrv_beccs_to_prodseq_patch(p) = cropother_to_cropprod_patch(p)
    end do
    ...
    call p2g(bounds, &
         this%hrv_beccs_to_prodseq_patch(bounds%begp:bounds%endp), &
         this%hrv_beccs_to_prodseq_grc(bounds%begg:bounds%endg), &
         p2c_scale_type = 'unity', &
         c2l_scale_type = 'unity', &
         l2g_scale_type = 'unity')
    ! Determine gains from dynamic landcover
  ...
  end subroutine PartitionCropProdFluxes
