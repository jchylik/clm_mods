...
contains
  ...
  subroutine set_old_weights(this, bounds)
    ...
    do p = bounds%begp, bounds%endp
       c = patch%column(p)
       this%pwtgcell_old(p) = patch%wtgcell(p)
    ...
    do fp = 1, num_filterp_with_inactive
       ...
       if (this%dwt(p) > 0._r8) then
          ...
          if (present(flux_out_col_area)) then
             ...
             ! cwtgcell_old > 0.
             if (this%cwtgcell_old(c).gt.0._r8) then
               flux_out_col_area(p) = flux_out_col_area(p) + &
                  var(p) * (this%dwt(p) / this%cwtgcell_old(c))
             else
               ! for situation when cwtgcell_old is 0 dur to being unknown or unused
               flux_out_col_area(p) = flux_out_col_area(p)     ! possibly later modify
             end if
          end if
       end if
    end do
